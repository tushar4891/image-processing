...
router.post("/change-saturation", express.json(), (req, res) => {
  console.log("Inside /change-saturation !!!! ", req.body);

  const { imagePath, saturation } = req.body;

  if (!imagePath || !saturation) {
  ...
  const worker = new Worker("./routes/saturationWorker.js", {
    workerData: { imagePath, saturation },
  });
  ...
  const fullQualityOutputPath = `./upload/images/saturation_${path.basename(
    imagePath
  )}`;
